...
# Railway uses the repo root as build context; this file is read from
# services/cards-service/ but all COPY paths are relative to repo root.
...
COPY --from=health-check-builder /home/packages/health-check ./health-check
WORKDIR /home/service/cards-service
COPY --from=builder /home/service/cards-service/build ./build
COPY services/cards-service/package.json services/cards-service/yarn.lock ./
